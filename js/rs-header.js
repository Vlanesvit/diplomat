...
function headerFixed() {
	...
	function headerClassAdd() {
		const topheader = header.querySelector('.rs-header__top').clientHeight;
		header.classList.toggle('_header-scroll', window.scrollY > topheader);
	}
...
==================================== */
function search() {
	const searchsHeader = document.querySelectorAll('.rs-search__btn');
	searchsHeader.forEach(search => {
		const searchInput = search.querySelector('.rs-search__input')
		const searchForm = search.querySelector('.rs-search__form');

		search.addEventListener('click', function (e) {
			e.preventDefault();
			// Первый клик - открывает поиск. Если в поиске есть что-то, то отправляем форму
			if (searchInput.value != '') {
				searchForm.submit();
			} else {
				search.classList.toggle('_active-search');
